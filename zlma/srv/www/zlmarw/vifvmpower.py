...
class Vif_img_set:
  ...
  def iucv_ping_server(self, user_id: str): # ping z/VM guest using IUCV
    """
    Ping a z/VM guest using IUCV communication
    Returns 0 if successful, 1 if failed
    """
    user_id = user_id.upper()              # z/VM user IDs are typically uppercase
    # Use 'true' command which should exist on all Linux systems and always returns 0
    iucv_ping_cmd = f"/opt/zthin/bin/IUCV/iucvclnt {user_id} /bin/true"
    try:
      proc = subprocess.run(iucv_ping_cmd, shell=True, capture_output=True, text=True, timeout=30)
      return proc.returncode               # 0 = IUCV connection successful
    except subprocess.TimeoutExpired:
      return 1                             # timeout = connection failed
    except Exception:
      return 1                             # any other exception = connection failed
  ...
  def create_page(self):                   # make the HTML page
    ...
    for next_row in row_list:
      ...
      ping_result = self.ping_server(ip_addr) # try regular ping first
      if ping_result:                      # regular ping failed, try IUCV
        iucv_result = self.iucv_ping_server(user_id) # try IUCV ping
        if iucv_result:                    # both ping methods failed
          html += '<td style="color:red;">down</td>'
          status = "down"
        else:                              # IUCV ping succeeded
          html += '<td style="color:orange;">up (IUCV)</td>'
          status = "up"
      else:                                # regular ping succeeded
